...
public class Point2D{
    ...
    public void scale(Point2D cen, double ratio) {
        // Calculate the new x and y coordinates of the object
        double newX = cen._x + (this._x - cen._x) * ratio;
        double newY = cen._y + (this._y - cen._y) * ratio;
    ...
    public void rotate(Point2D cen, double angleDegrees) {
		if(angleDegrees % 360 == 0 || cen._x == this._x && cen._y == this._y) return;
        else{
            double angleRadians = Math.toRadians(angleDegrees);
            double cosAngle = Math.cos(angleRadians);
            double sinAngle = Math.sin(angleRadians);

            double x = this._x - cen._x;
            double y = this._y - cen._y;

            this._x = x * cosAngle - y * sinAngle + cen._x;
            this._y = x * sinAngle + y * cosAngle + cen._y;
        }
	}
}
