...
/*
 * credits:
 * i used the help of https://chat.openai.com/chat to write Scale and Rotate
 */
...
public class Point2D{
	...
	public void scale(Point2D cen, double ratio) {
		...
		_x = cen._x + (this._x - cen._x ) * ratio;
		_y = cen._y + (this._y - cen._y ) * ratio;
		
	/////////////////////////////////////////
	}

	public void rotate(Point2D cen, double angleDegrees) {
		double dx = cen.x()-this._x;
		double dy = cen.y()-this._y;
		double r = Math.sqrt((Math.pow(dx,2)+Math.pow(dy,2)));
		double deg = Math.atan2(dy, dx);
		double newpx = r*Math.cos(deg + angleDegrees) + cen.x();
		double newpy = r*Math.sin(deg + angleDegrees) + cen.y();
		this._x = newpx;
		this._y = newpy;

	}
	public double calcAngel(Point2D cen, Point2D _p1) {
		Point2D _p2 = new Point2D(_p1.x(), cen.y());
		double disx = cen.x() - _p2.x();
		double disy = _p2.y() - _p1.y();
		double alpha = Math.atan2(disy, disx); 
		rotate(cen, alpha);
		return alpha;
	}
